...
/**
 * Аутентификация сотрудника it-отдела — двумя способами, чтобы одни и те
 * же эндпоинты обслуживали и Telegram Mini App, и отдельное веб-приложение
 * IT-отдела (/it/), без дублирования бизнес-логики:
 *  1. Telegram Mini App — заголовок X-Init-Data (initData WebApp).
 *  2. Обычная веб-сессия (ФИО+пароль) — используется /it/, когда
 *     заголовка X-Init-Data нет.
 * Возвращает строку users или завершает запрос с ошибкой.
 */
